...
const PlaylistPage = () => {
  ...
  const isCurrentPlaylistPlaying =
    currentPlaylist.songs.some((s) => s._id === currentSong?._id) && isPlaying;

  const handlePlayPause = () => {
    if (isCurrentPlaylistPlaying) {
      togglePlay();
    } else {
      playAlbum(currentPlaylist.songs as any, 0);
    }
  };
    ...
    <div className='h-full rounded-md overflow-hidden'>
      <ScrollArea className='h-full'>
        ...
        <div className='px-6 pb-6 bg-zinc-900'>
          {/* Table header */}
          <div className='grid grid-cols-[16px_4fr_2fr_1fr] gap-4 px-4 py-2 text-sm text-zinc-400 border-b border-zinc-800 mb-2'>
            <div>#</div>
            <div>Title</div>
            <div>Artist</div>
            ...
            <div className='space-y-1'>
              {currentPlaylist.songs.map((song, index) => {
                ...
                return (
                  <div
                    key={song._id}
                    onClick={() => playAlbum(currentPlaylist.songs as any, index)}
                    className={`grid grid-cols-[16px_4fr_2fr_1fr] gap-4 px-4 py-2 text-sm rounded-md group cursor-pointer hover:bg-zinc-800 ${
